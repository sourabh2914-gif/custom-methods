 ...
 * description: Generate random alphanumeric with count ${count} prefix ${prefix} suffix ${suffix} and store in $[result]
...
export async function generateRandomAlphanumeric(ctx: WalnutContext) {
  // ctx.args[0] = value of ${count} — number of random alphanumeric characters to generate
  ...
  ctx.log(`Generated random alphanumeric: "${result}" (count: ${count}, prefix: "${prefix}", suffix: "${suffix}")`);
