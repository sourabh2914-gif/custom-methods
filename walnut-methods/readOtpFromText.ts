...
export async function readOtpFromText(ctx: WalnutContext) {
  // ctx.args[0] = value of ${textMessage} — the full SMS/text content
  // ctx.args[1] = "otp_code" (from $[otp_code]) — runtime variable name to store the extracted OTP
  const textMessage = ctx.args[0];
  const outputVar = ctx.args[1];

  ctx.log(`Extracting OTP from text: "${textMessage}"`);

  // Step 1: Find all numeric sequences in the text message
  const allNumbers = textMessage.match(/\d+/g) || [];
  ...
  if (otpCandidates.length === 0) {
    throw new Error(`No OTP found in text message: "${textMessage}". Expected a 4–8 digit numeric code.`);
  }

  // Step 4: Pick the most likely OTP using scoring heuristics
  // Priority: proximity to OTP-related keywords > shorter length (OTPs are usually 4–6 digits)
  const otpKeywords = ['otp', 'code', 'verification', 'verify', 'pin', 'password', 'one-time', 'token', 'passcode'];
  const lowerText = textMessage.toLowerCase();

  let bestCandidate = otpCandidates[0];

  // Check if any candidate appears near an OTP-related keyword
  for (const keyword of otpKeywords) {
    const keywordIndex = lowerText.indexOf(keyword);
    if (keywordIndex === -1) continue;

    // Find the candidate closest (within 30 chars) to the keyword
    for (const candidate of otpCandidates) {
      const candidateIndex = textMessage.indexOf(candidate, Math.max(0, keywordIndex - 10));
      if (candidateIndex !== -1 && Math.abs(candidateIndex - keywordIndex) <= 30) {
